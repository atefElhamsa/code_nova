...
export default function AdminPage() {
  ...
  const load = useCallback(async (p: string) => {
    ...
    try {
      ...
      if ((e as { message?: string })?.message?.includes('Unauthorized')) { setAuthed(false); setAuthErr('كلمة المرور غلط'); }
  ...
  const handleGenerateNewCode = async () => {
    ...
    if (!error) {
      ...
      alert("حدث خطأ أثناء توليد الكود. تأكد من إعدادات قاعدة البيانات.");
    }
          ...
          <div className="body scroll">
            ...
            <div className="toolbar">
              ...
              <div className="tabs">
                {[
                  ...
                  { key: 'codes', lbl: `أكواد التفعيل`, cls: 'b0' },
                ].map(t => (
                  <button key={t.key}
            ...
            {tab === 'codes' ? (
              <div className="card" style={{ padding: '0', overflow: 'hidden' }}>
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', padding: '20px 26px', background: 'rgba(255,255,255,.013)', borderBottom: '1px solid rgba(255,255,255,.055)' }}>
                <h3 style={{ color: '#e2e8f0', margin: 0, marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '1rem', fontWeight: 800 }}>
                  <svg width="20" height="20" style={{ color: '#38bdf8' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>
                  إدارة أكواد التفعيل
                </h3>

                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(255,255,255,0.03)', padding: '6px 12px', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <span style={{ color: '#94a3b8', fontSize: '0.8rem', fontWeight: 700 }}>إنشاء لـ:</span>
                  <select
                    value={selectedCourse}
                    onChange={(e) => setSelectedCourse(e.target.value)}
                    style={{ background: 'transparent', border: 'none', color: '#e2e8f0', outline: 'none', fontFamily: 'inherit', fontWeight: 700, fontSize: '0.85rem' }}
                  >
                ...
                <button
                  ...
                  className="login-btn"
                  style={{ padding: '8px 18px', width: 'auto', margin: 0, borderRadius: '12px', fontSize: '0.85rem' }}
                >
                  {isGenerating ? 'جاري التوليد...' : '✨ إنشاء كود'}
                </button>
                <button onClick={fetchCodes} className="hbtn" style={{ height: '38px', borderRadius: '12px' }}>
                  🔄 تحديث
                </button>
              </div>
              ...
              {codesLoading ? (
                <div className="empty">
                  <div className="spin" style={{ width: 24, height: 24, border: '3px solid rgba(255,255,255,.2)', borderTopColor: '#38bdf8', margin: '0 auto' }} />
                </div>
              ) : (
                <div style={{ overflowX: 'auto', paddingBottom: '8px' }}>
                  <div className="grid-tbl">
                    ...
                    {accessCodes.length === 0 ? (
                      <div className="empty">
                        <div className="empty-txt">لا توجد أكواد تفعيل حتى الآن</div>
                      </div>
                      ...
                      return (
                        <div key={codeRow.id} className="grid-row" style={{ gridTemplateColumns: '25% 22% 13% 15% 25%', animationDelay: `${i * .04}s` }}>
                          ...
                          <div className="grid-cell" style={{ textAlign: 'center' }}>
                            {isStudentReq ? (
                              <span style={{ color: '#f59e0b', fontSize: '0.82rem', fontWeight: 700 }}>👨‍🎓 طالب</span>
                            ) : (
                              <span style={{ color: '#a855f7', fontSize: '0.82rem', fontWeight: 700 }}>👑 أدمن</span>
                            )}
                          </div>
                          <div className="grid-cell" style={{ textAlign: 'center' }}>
                            {codeRow.is_approved ? (
                              <span className={isStudentReq ? "sbadge ok" : "sbadge bad"}>
                                <span className="sbadge-dot" /> {isStudentReq ? 'مفعل' : 'محروق'}
                              </span>
                            ) : (
                              <span className="sbadge" style={{ background: isStudentReq ? 'rgba(251,191,36,.1)' : 'rgba(148,163,184,.1)', color: isStudentReq ? '#fbbf24' : '#94a3b8', border: isStudentReq ? '1px solid rgba(251,191,36,.2)' : '1px solid rgba(148,163,184,.2)' }}>
                                {isStudentReq ? <><span className="sbadge-dot" style={{ background: '#fbbf24' }} /> مراجعة</> : <span style={{ display: 'inline-block' }}>متاح</span>}
                              </span>
                            )}
                          </div>
                          <div className="grid-cell" style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                            {isStudentReq && !codeRow.is_approved && (
                              <button onClick={() => handleApproveStudent(codeRow.id)} className="abtn unblock">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg> تفعيل
                              </button>
                            )}
                            {!isStudentReq && (
